...
var filtered = myNumbers.filter(num => {
	if(num>100)
	return num;
})
...
var total = myNumbers.reduce((total, amount) => {
	return total + amount;
})
...
// myNumbers.forEach(function(arr, i){
// 	for(let i=0; i<arr.length; i++)
// });
...
var forTheLoveOfGeorge = notGeorge.map(item => item = 'George');
...
var enemies = people.filter(function(item){
...
var totallyAwesome = people.reduce((total, amount) => {
	return total + amount;
});
